	...
	<body>
		...
		<div id="show-curso" class="content scaffold-show" role="main">
			...
			<ol class="property-list curso">
				...
				<g:if test="${cursoInstance?.estudiante}">
				<li class="fieldcontain">
					<span id="estudiante-label" class="property-label"><g:message code="curso.estudiante.label" default="Estudiante" /></span>
					
						<span class="property-value" aria-labelledby="estudiante-label"><g:link controller="alumno" action="show" id="${cursoInstance?.estudiante?.id}">${cursoInstance?.estudiante?.encodeAsHTML()}</g:link></span>
